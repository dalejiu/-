...
if __name__ == "__main__":
    ...
    #需求7行业平均薪资
    # 对 jobData 按照 'type' 列进行分组，计算 'maxSalary' 列的平均值，并将结果命名为 'avg_max_salary'
    result7 = jobData.groupBy("type").agg(F.avg("maxSalary").alias("avg_max_salary"))
    # 将结果写入 MySQL 数据库
    result7.write.mode("overwrite"). \
        ...
        option("driver", "com.mysql.jdbc.Driver"). \
        save()
    # 将结果保存为 Parquet 格式的表 'averageType'
    result7.write.mode("overwrite"). \
        format("parquet"). \
        saveAsTable("averageType")
    # 使用 Spark SQL 查询并显示 'averageType' 表的内容
